...
//		"dom": '<"top">rt<"bottom"flp><"clear">',
		...
		"aoColumnDefs": [{                      // 指定列不参与排序
		    "orderable": false,
		    "aTargets": [0,1,2,3,4,5,6]                   // 对应你的表格的列数
		}],
		"columns": [                            // 自定义数据列
			...
			{data: function(obj) {
				if (obj.contactor != null) {
					return obj.contactor;
				}
				return '--';
			}},
			{data: function(obj) {
		...
		"fnRowCallback" : function(row, data, index) {
			...
			laytpl(opbtntpl).render(data, function(html){
				$('td:eq(8)', row).html($.trim(html) == '' ? '--' : html);
			});
...
//		});
		var index = layer.open({
			type: 2,
			title: '添加用户',
			content: WEB_ROOT + "/admin/user/edit"
		});
		layer.full(index);
		...
		if(ids == null || ids == ''){
			...
			jacommon.confirm('确认要启用选中的数据吗？', function() {
				var postUrl = WEB_ROOT + '/admin/user/enable?t=' + new Date().getTime();
		...
		if(ids == null || ids == ''){
			...
			jacommon.confirm('确认要重置选中的密码吗？', function() {
				var postUrl = WEB_ROOT + '/admin/user/resetPassword?t=' + new Date().getTime();
				var postData = {'ids' : ids};
				// 提交请求
				jacommon.ajaxPost(postUrl, postData, function(res) {
					jacommon.success('密码重置成功', function() {
						refresh();
					});
				}, function(res) {
					jacommon.error('密码重置失败，' + res.message + '(错误代码：' + res.code + ')');
				}, function() {
